...
export const getDashboardMetrics = async (rawArgs, context) => {
    ...
    const scansThisMonth = allScans.filter((s) => new Date(s.createdAt) > thirtyDaysAgo).length;
...
// Get quota status
export const getQuotaStatus = async (_rawArgs, context) => {
    const user = context.user;
    if (!user) {
        throw new HttpError(401, "Only authenticated users are allowed to perform this operation");
    }
    ...
    const scansLastSevenDays = scans.filter((s) => new Date(s.createdAt) >= sevenDaysAgo).length;
    const limit = user.subscriptionStatus === "pro" ? 1000 : 100;
    let trend = "stable";
...
export const getRecentScans = async (rawArgs, context) => {
    const { limit = 10 } = ensureArgsSchemaOrThrowHttpError(getRecentScansInputSchema, rawArgs);
    const user = context.user;
    if (!user) {
        throw new HttpError(401, "Only authenticated users are allowed to perform this operation");
    }
...
export const getSeverityBreakdown = async (rawArgs, context) => {
