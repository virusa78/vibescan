 ...
 */
export async function webhookDeliveryWorker(job) {
    const data = job.data;
    ...
    console.log(`[WebhookWorker] Processing delivery job: webhook=${webhookId}, scan=${scanId}, target=${targetHost}, attempt=${attemptNumber}`);
    try {
        ...
        try {
            ...
            console.warn(`[WebhookWorker] HTTP error for ${targetHost}: ${error instanceof Error ? error.message : String(error)}`);
            httpStatus = 0;
        ...
        console.error(`[WebhookWorker] Job failed: ${error instanceof Error ? error.message : String(error)}`);
        throw error;
    }
}
/**
...
//# sourceMappingURL=webhookDeliveryWorker.js.map