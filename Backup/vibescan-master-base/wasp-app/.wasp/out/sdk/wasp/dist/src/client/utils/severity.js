...
export function calculateSeverityBreakdown(scans) {
    const breakdown = {
        critical: 0,
        high: 0,
        medium: 0,
 ...
 */
export function getSeverityBorderColor(severity) {
    switch (severity) {
        case 'critical':
 ...
 */
export function getScanTypeDisplay(inputType) {
    const normalized = inputType?.toLowerCase();
    if (normalized === 'github' || normalized === 'github_app')
        return 'GitHub';
    if (normalized === 'sbom' || normalized === 'sbom_upload')
        return 'SBOM';
    if (normalized === 'source_zip' || normalized === 'zip')
        return 'ZIP';
    if (normalized === 'ci_plugin' || normalized === 'ci')
        return 'CI/CD';
