...
import { buildDashboardSearch, normalizeStatusValue, parseDashboardSearch, } from './urlState';
import { isEditableTarget } from '../client/utils/keyboard';
import { useQuery, getOnboardingState, bulkCancelScans, bulkRerunScans, createScanSavedView, deleteScanSavedView, exportScans, getQuotaStatus, getRecentScans, getSeverityBreakdown, getTrendSeries, listScanSavedViews, updateScanSavedView, submitScan, } from 'wasp/client/operations';
...
export default function DashboardPage() {
    ...
    useEffect(() => {
        const timer = setTimeout(() => {
            ...
            const nextSearch = buildDashboardSearch(parsedSearch.sortField, parsedSearch.sortDirection, parsedSearch.statuses, nextQuery);
    ...
    useEffect(() => {
        run(async () => {
            ...
            const formattedScans = (scansData.scans || []).map((scan) => {
    ...
    const metrics = useMemo(() => {
        ...
        const avgSeverity = severity.critical > 0
            ? 'Critical'
            : severity.high > 0
                ? 'High'
                : severity.medium > 0
    ...
    const emptyStateConfig = useMemo(() => {
        ...
        if (onboarding?.should_show_onboarding) {
            return {
                title: 'Start your first workspace scan',
                description: onboarding.workspace_name
                    ? `${onboarding.workspace_name} is ready. Use the guided setup to pick the fastest path to your first result.`
                    : 'Use the guided setup to pick the fastest path to your first result.',
                actionLabel: 'Open guided setup',
                actionRoute: '/onboarding',
                secondaryActionLabel: 'Open manual scan',
                secondaryActionRoute: '/new-scan?type=github&source=dashboard',
            };
        }
        if (onboarding?.has_github_installation) {
            return {
                title: 'No scans in this workspace yet',
                description: 'GitHub access is connected. Queue your first repository scan or inspect another input source.',
                actionLabel: 'Scan a GitHub repo',
                actionRoute: '/new-scan?type=github&source=dashboard',
                secondaryActionLabel: 'Use SBOM instead',
                secondaryActionRoute: '/new-scan?type=sbom&source=dashboard',
            };
        }
        return {
            title: 'No scans yet',
            description: 'Submit your first vulnerability scan to populate the dashboard and unlock trend views.',
            actionLabel: 'Create first scan',
            actionRoute: '/new-scan?type=github&source=dashboard',
            secondaryActionLabel: 'Open onboarding',
            secondaryActionRoute: '/onboarding',
        };
    }, [onboardingState]);
    const tableScans = useMemo(() => scans.map((scan) => ({
        id: scan.id,
        status: scan.status,
        inputType: scan.inputType,
        inputRef: scan.inputRef,
    ...
    const [categories, scansSeries, findingsSeries] = useMemo(() => {
        ...
        return [dayList, scansSeries, findingsSeries];
    }, [scans, timeRange]);
    const statCards = [
        {
            label: 'TOTAL SCANS',
    ...
    const handleCopyScanId = async (scanId) => {
        try {
    ...
    const handleBulkCancel = async (scanIds) => {
        try {
    ...
    const handleBulkRerun = async (scanIds) => {
        try {
    ...
    const handleBulkExport = async (scanIds, format) => {
        try {
    ...
    const handleApplySavedView = () => {
        ...
        const nextSearch = buildDashboardSearch(selectedView.sortField, selectedView.sortDirection, selectedView.statuses, selectedView.query);
        navigate({ pathname: location.pathname, search: nextSearch }, { replace: false });
    };
    ...
    const hasAnyScans = useMemo(() => Object.values(statusCounts).reduce((sum, count) => sum + count, 0) > 0, [statusCounts]);
    return (<div className="p-8 lg:p-10">
      <div className="mb-8 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
        ...
        <div className="inline-flex items-center gap-2 rounded-md border border-border/60 bg-card/40 p-1">
          {['7d', '30d', 'all'].map((range) => (<button key={range} type="button" className={`rounded px-3 py-1 text-xs font-medium transition ${timeRange === range
                ? 'bg-primary text-primary-foreground'
                : 'text-muted-foreground hover:text-foreground'}`} onClick={() => setTimeRange(range)} aria-pressed={timeRange === range} aria-label={`Set time range ${range.toUpperCase()}`}>
              {range.toUpperCase()}
            </button>))}
        </div>
      </div>

      {error && (<div className="mb-6 rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-600">
          {error}
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
        {statCards.map((stat) => (<MetricCard key={stat.label} label={stat.label} value={stat.value} subtext={stat.subtext} icon={stat.icon} trend={stat.trend} loading={isLoading}/>))}
      </div>
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur-sm mb-8">
        ...
        <CardContent>
          {isLoading ? (<div className="space-y-2">
              ...
              <div className="h-4 bg-muted rounded animate-pulse"/>
              <div className="h-4 bg-muted rounded animate-pulse"/>
            </div>) : visibleTrendBuckets.length === 0 ? (<p className="text-sm text-muted-foreground">No trend data available for selected range.</p>) : (<div className="space-y-3">
              {visibleTrendBuckets.map((bucket) => {
                ...
                return (<div key={bucket.bucket_start} className="space-y-1">
              ...
              {isLoading ? (<div className="space-y-2">
                  ...
                  <div>
                    ...
                    <div className="w-full bg-border/30 rounded-full h-2">
                      <div className="bg-primary rounded-full h-2 transition-all" style={{ width: `${Math.min(quota.percentage, 100)}%` }}/>
                    </div>
                  </div>
                  {quota.monthly_reset_date && (<p className="text-xs text-muted-foreground">
      ...
      <Card className="border-border/50 bg-card/50 backdrop-blur-sm mb-6">
        ...
        <CardContent className="flex flex-col gap-3 md:flex-row md:items-center">
          <input value={savedViewName} onChange={(event) => setSavedViewName(event.target.value)} placeholder="View name" className="w-full md:w-56 rounded border border-border/60 bg-background px-3 py-2 text-sm"/>
          <button type="button" onClick={handleSaveCurrentView} className="rounded border border-primary/50 px-3 py-2 text-sm text-primary hover:bg-primary/10">
            Save Current
          </button>
          <select value={activeSavedViewId} onChange={(event) => setActiveSavedViewId(event.target.value)} className="w-full md:w-64 rounded border border-border/60 bg-background px-3 py-2 text-sm">
            <option value="">Select saved view</option>
            {savedViews.map((view) => (<option key={view.id} value={view.id}>
                {view.name}
              </option>))}
          </select>
          <button type="button" disabled={!activeSavedViewId} onClick={handleApplySavedView} className="rounded border border-border/60 px-3 py-2 text-sm disabled:opacity-50">
            Apply
          </button>
          <button type="button" disabled={!activeSavedViewId} onClick={handleUpdateSavedView} className="rounded border border-border/60 px-3 py-2 text-sm disabled:opacity-50">
            Update
          </button>
          <button type="button" disabled={!activeSavedViewId} onClick={handleDeleteSavedView} className="rounded border border-red-500/40 px-3 py-2 text-sm text-red-500 disabled:opacity-50">
            Delete
          </button>
        </CardContent>
      </Card>

      {!hasAnyScans && !isLoading ? (<EmptyState title={emptyStateConfig.title} description={emptyStateConfig.description} actionLabel={emptyStateConfig.actionLabel} actionRoute={emptyStateConfig.actionRoute} secondaryActionLabel={emptyStateConfig.secondaryActionLabel} secondaryActionRoute={emptyStateConfig.secondaryActionRoute}/>) : (<ScanTable scans={tableScans} loading={isLoading} onRefresh={handleRefresh} sortField={parsedSearch.sortField} sortDirection={parsedSearch.sortDirection} onSortChange={handleSortChange} statusFilters={parsedSearch.statuses} statusCounts={statusCounts} onToggleStatus={handleToggleStatus} searchQuery={searchInputValue} onSearchQueryChange={setSearchInputValue} searchInputRef={searchInputRef} filteredCount={filteredCount} totalCount={totalCount} onCancelScan={handleCancelScan} onRerunScan={handleRerunScan} onCopyScanId={handleCopyScanId} onBulkCancel={handleBulkCancel} onBulkRerun={handleBulkRerun} onBulkExport={handleBulkExport}/>)}
    </div>);
}
