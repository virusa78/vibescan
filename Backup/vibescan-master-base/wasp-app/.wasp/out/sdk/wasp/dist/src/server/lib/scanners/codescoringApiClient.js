...
class CodescoringSshClient {
    ...
    runScpToRemote(localPath, remotePath) {
        const args = [
            '-o', 'BatchMode=yes',
            '-o', 'StrictHostKeyChecking=no',
            '-i', this.keyPath,
    ...
    runScpFromRemote(remotePath, localPath) {
        const args = [
            '-o', 'BatchMode=yes',
            '-o', 'StrictHostKeyChecking=no',
            '-i', this.keyPath,
 ...
 */
function generateMockFindings(components) {
 ...
 */
function parseCodescoringResponse(apiResponse) {
    if (!apiResponse) {
        return [];
    }
    const findings = [];
            ...
            for (const vuln of apiResponse.vulnerabilities) {
                ...
                for (const affect of affects) {
                    ...
                    findings.push({
                        ...
                        severity: (rating?.severity || 'info').toLowerCase(),
 ...
 */
export async function scanWithCodescoring(components, scanId, input) {
    const run = await scanWithCodescoringDetailed(components, scanId, input);
    return run.findings;
}
export async function scanWithCodescoringDetailed(components, scanId, input) {
