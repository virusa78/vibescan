...
import { buildCycloneDxSbom } from '../../services/inputAdapterService.js';
import { shellQuote, runRemoteCommandViaSsh } from './remoteSsh.js';
import { getSnykCredentialMode, getSnykTimeoutMs, } from '../../config/runtime.js';
function defaultLocalExecutor(command, args, input, timeoutMs, env = process.env) {
...
function getSnykRuntimeConfig() {
    ...
    return {
        mode: getRuntimeMode(),
        commandTemplate: process.env.SNYK_COMMAND?.trim()
...
function buildCredentialsFromResolved(resolvedCredentials) {
...
function buildNormalizedFindings(rawOutput) {
...
function resolveSnykResult(result) {
...
function buildEnvForCredentials(credentials) {
    return {
        ...process.env,
...
function buildSshShellCommand(commandTemplate, remoteBomPath, credentials) {
    const exports = [
...
export async function runSnykScan(components, scanId, resolvedCredentials, localExecutor = defaultLocalExecutor, sshExecutor) {
    ...
    if (config.mode === 'mock') {
        ...
        rawOutput = runSnykViaSsh(bomJson, credentials, { ...config.ssh, commandTemplate: config.commandTemplate, remoteTempDir: config.remoteTempDir, timeoutMs: config.timeoutMs }, scanId, sshExecutor);
    }
...
//# sourceMappingURL=snykRuntime.js.map