...
import { generateCveRemediation, generateReportPDF, getCIDecision, getReport, getReportSummary, upsertFindingAnnotation, } from 'wasp/client/operations';
...
function isFixableFinding(finding) {
...
export default function ReportsPage() {
    ...
    const [ciDecision, setCiDecision] = useState(null);
    ...
    const [remediationTimestamp, setRemediationTimestamp] = useState({});
    useEffect(() => {
        if (!scanId) {
    ...
    const severitySummary = useMemo(() => {
        const fallback = findings.reduce((acc, finding) => {
    ...
    const filteredFindings = useMemo(() => {
        ...
        if (sortBy === 'severity') {
            ...
            filtered.sort((a, b) => order[(a.severity ?? 'unknown').toLowerCase()] - order[(b.severity ?? 'unknown').toLowerCase()]);
        }
    ...
    const buildPackageLink = (finding) => {
    ...
    const updateFindingAnnotationInState = (findingId, annotation) => {
        setReport((previous) => {
            if (!previous)
                return previous;
            if (Array.isArray(previous.findings)) {
                return {
                    ...previous,
                    findings: previous.findings.map((finding) => finding.id === findingId ? { ...finding, annotation } : finding),
                };
            }
            if (Array.isArray(previous.vulnerabilities)) {
                return {
                    ...previous,
                    vulnerabilities: previous.vulnerabilities.map((finding) => finding.id === findingId ? { ...finding, annotation } : finding),
                };
            }
            return previous;
        });
    ...
    if (isLoading) {
        return (<div className="p-8 lg:p-10 space-y-6">
        <div className="space-y-2">
          <Skeleton className="h-10 w-40"/>
          <Skeleton className="h-4 w-64"/>
        </div>
        <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
          {[0, 1, 2, 3].map((idx) => (<Card key={idx} className="border-border/50 bg-card/50 backdrop-blur-sm">
        ...
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent className="space-y-3">
            {[0, 1, 2].map((idx) => (<Skeleton key={idx} className="h-14 w-full"/>))}
          </CardContent>
        </Card>
      </div>);
    }
    ...
    return (<div className="p-8 lg:p-10">
      ...
      <div className="grid grid-cols-1 md:grid-cols-4 gap-4 mb-8">
        <Card className="border-border/50 bg-card/50 backdrop-blur-sm">
          ...
          <CardContent>
            <p className="text-3xl font-bold text-foreground">{summary?.totalFindings ?? reportFindings.length}</p>
          </CardContent>
        </Card>
      ...
      {pdfStatus && (<div className="mb-6 rounded-md border border-border/60 bg-card/40 p-3 text-sm text-muted-foreground">
          {pdfStatus}
        </div>)}
        ...
        <CardHeader className="space-y-3">
          <div className="flex flex-col md:flex-row md:items-center md:justify-between gap-2">
            ...
            <div className="flex items-center gap-2">
              <Input ref={searchInputRef} placeholder="Search by CVE or package" value={searchQuery} onChange={(event) => setSearchQuery(event.target.value)} className="w-64" aria-label="Search findings"/>
          ...
          <div className="space-y-2">
            ...
            <ToggleChipGroup options={severityChipOptions} value={severityFilter} onChange={(next) => setSeverityFilter(next)} ariaLabel="Severity filters"/>
          </div>

          <div className="space-y-2">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">Fixability</p>
            <ToggleChipGroup options={fixableChipOptions} value={fixableFilter} onChange={(next) => setFixableFilter(next)} ariaLabel="Fixability filters"/>
          </div>

          <div className="space-y-2">
            <p className="text-xs text-muted-foreground uppercase tracking-wide">Annotation</p>
            <ToggleChipGroup options={annotationChipOptions} value={annotationFilter} onChange={(next) => setAnnotationFilter(next)} ariaLabel="Annotation filters"/>
          </div>
        </CardHeader>
        <CardContent>
          {findings.length === 0 ? (<div className="rounded-md border border-border/60 bg-card/40 p-4">
            ...
            </div>) : filteredFindings.length === 0 ? (<div className="rounded-md border border-border/60 bg-card/40 p-4 text-sm text-muted-foreground flex items-center justify-between gap-3">
              <div className="flex items-center gap-2">
              ...
              {filteredFindings.map((finding, index) => {
                ...
                return (<div key={fid} className="rounded-md border border-border/50 p-3">
                    <div className="flex items-center justify-between gap-3">
                      <p className="font-mono text-sm text-foreground">
                        {cve ? (<span className="flex items-center gap-2">
                            <ExternalLink href={buildGitHubAdvisoryUrl(cve)} aria-label={`Open GitHub advisory search for ${cve}`}>
                              {cve}
                            </ExternalLink>
                            <ExternalLink href={buildNvdUrl(cve)} withIcon={false} className="text-xs text-muted-foreground" aria-label={`Open NVD details for ${cve}`}>
                              NVD
                            </ExternalLink>
                          </span>) : ('Unknown CVE')}
                      </p>
                    ...
                    <p className="mt-1 text-sm text-muted-foreground">
                      {finding.packageName ? (packageLink ? (<ExternalLink href={packageLink} className="text-sm" aria-label={`Open package page for ${finding.packageName}`}>
                            {finding.packageName}
                          </ExternalLink>) : (finding.packageName)) : (finding.description ?? 'No details')}
                    </p>
                    <div className="mt-2 flex flex-wrap items-center gap-2 text-xs">
                      ...
                      {finding.annotation?.reason ? (<span className="text-muted-foreground">Note: {finding.annotation.reason}</span>) : null}
                    </div>
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      <Select value={draft.state} onValueChange={(value) => setAnnotationDrafts((previous) => ({
                        ...previous,
                        [fid]: { ...draft, state: value },
                    }))}>
                      ...
                      <Input value={draft.reason} onChange={(event) => setAnnotationDrafts((previous) => ({
                        ...previous,
                        [fid]: { ...draft, reason: event.target.value },
                    }))} className="w-64" placeholder="Optional note" aria-label="Annotation reason"/>
                      {draft.state === 'snoozed' && (<Input type="date" value={draft.expiresAt} onChange={(event) => setAnnotationDrafts((previous) => ({
                            ...previous,
                            [fid]: { ...draft, expiresAt: event.target.value },
                        }))} className="w-44" aria-label="Snooze until date"/>)}
                    ...
                    <div className="mt-2 flex flex-wrap items-center gap-2">
                      ...
                      <Button disabled={!canCopyPatch} title={!canCopyPatch ? 'Patch snippet requires package and fixed version' : 'Copy patch snippet'} onClick={() => {
                        if (!canCopyPatch)
                            return;
                      ...
                      {remediationTimestamp[fid] && (<span className="text-xs text-muted-foreground">
...
//# sourceMappingURL=ReportsPage.jsx.map