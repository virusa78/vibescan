...
import { listApiKeys, generateApiKey, revokeApiKey } from 'wasp/client/operations';
...
import { getApiKeyDetails } from './client';
function formatIsoDate(value) {
    if (!value)
        return 'Never';
    return new Date(value).toLocaleDateString();
}
function formatRelative(value, emptyLabel) {
    if (!value)
        return emptyLabel;
...
function UsageChart({ data }) {
    ...
    if (points.length === 0) {
        return (<div className="rounded-lg border border-dashed p-5 text-sm text-muted-foreground">
    ...
    return (<div className="overflow-x-auto">
      <div className="grid min-w-[42rem] grid-cols-14 gap-2">
        {points.map((point) => {
            ...
            return (<div key={point.date} className="flex h-48 flex-col items-center justify-end gap-2">
              <div className="text-[11px] font-medium text-foreground">{point.count}</div>
              <div className="flex w-full flex-1 items-end">
                <div className="w-full rounded-t-md bg-primary/80 transition-all" style={{ height: `${height}%` }} aria-hidden="true"/>
              </div>
...
export default function ApiKeysPage() {
    ...
    const { isLoading: isListLoading, error: listError, run: runList, setError: setListError, } = useAsyncState();
    const { isLoading: isDetailsLoading, error: detailsError, run: runDetails, setError: setDetailsError, } = useAsyncState();
    const selectedKey = useMemo(() => apiKeys.find((key) => key.id === selectedKeyId) ?? null, [apiKeys, selectedKeyId]);
    ...
    const loadSelectedKeyDetails = useCallback(async (keyId) => {
        await runDetails(async () => {
    ...
    const handleRevokeKey = async (keyId) => {
        ...
        await runList(async () => {
            ...
            const reload = await listApiKeys();
            setApiKeys(reload);
    ...
    return (<div className="mt-10 px-6 pb-12">
      <div className="mx-auto max-w-7xl space-y-6">
        <Card className="border-border/60 shadow-lg">
          <CardHeader className="space-y-4 border-b border-border/60 pb-6">
            ...
            {showNewKeyForm && !generatedKey && (<form onSubmit={handleGenerateKey} className="space-y-4 rounded-lg border border-border/60 bg-card/60 p-4">
                <div className="space-y-2">
                  <Label htmlFor="keyName">Key name</Label>
                  <Input id="keyName" placeholder="e.g. GitHub Actions CI" value={newKeyName} onChange={(event) => setNewKeyName(event.target.value)} disabled={isListLoading}/>
                </div>
          ...
          <CardContent className="space-y-6 pt-6">
            {listError && (<Alert variant="destructive">
                <AlertDescription>{listError}</AlertDescription>
              </Alert>)}
            {detailsError && (<Alert variant="destructive">
            ...
            {generatedKey && (<Alert className="border-green-600 bg-green-50 dark:bg-green-950">
                <AlertDescription>
            ...
            <div className="grid gap-6 xl:grid-cols-[minmax(0,1.15fr)_minmax(0,0.85fr)]">
              <section className="space-y-4">
                ...
                {isListLoading && apiKeys.length === 0 ? (<div className="space-y-3">
                    <KeySkeleton />
                    <KeySkeleton />
                    <KeySkeleton />
                  </div>) : apiKeys.length === 0 ? (<div className="rounded-lg border border-dashed border-border/70 p-8 text-center">
                ...
                return (<div key={key.id} role="button" tabIndex={0} onClick={() => setSelectedKeyId(key.id)} onKeyDown={(event) => {
              ...
              <aside className="space-y-4">
                <Card className="border-border/60 bg-card/80">
                  ...
                  <CardContent className="pt-6">
                    {isDetailsLoading ? (<DetailSkeleton />) : detailsError ? (<div className="rounded-lg border border-dashed border-destructive/40 bg-destructive/5 p-6 text-sm text-destructive">
                        {detailsError}
                      </div>) : selectedKey && selectedKeyDetails ? (<div className="space-y-5">
                        <div className="flex flex-wrap items-center gap-2">
                      ...
                      </div>) : (<div className="rounded-lg border border-dashed border-border/70 p-6 text-sm text-muted-foreground">
...
function InfoCard({ label, value }) {
    return (<div className="rounded-lg border border-border/60 bg-background/70 p-3">
      <p className="text-xs uppercase tracking-wide text-muted-foreground">{label}</p>
      <p className="mt-1 text-sm font-medium">{value}</p>
    </div>);
}
//# sourceMappingURL=ApiKeysPage.jsx.map