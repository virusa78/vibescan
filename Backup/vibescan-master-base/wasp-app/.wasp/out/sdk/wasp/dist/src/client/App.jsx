...
import { generateApiKey, getOnboardingState, useQuery } from 'wasp/client/operations';
...
function getFuzzyScore(query, source) {
...
export default function App() {
    ...
    const [paletteScans, setPaletteScans] = useState([]);
    ...
    const onboardingRedirectPath = useMemo(() => {
        if (isAuthLoading || !user || !onboardingState) {
            return null;
        }
        const pathname = location.pathname;
        const isOnboardingPage = pathname === routes.OnboardingRoute.to;
        const isDashboardLikePath = pathname === '/' || pathname === routes.DashboardRoute.to;
        if (onboardingState.should_show_onboarding && isDashboardLikePath) {
            return routes.OnboardingRoute.to;
        }
        if (!onboardingState.should_show_onboarding && isOnboardingPage) {
            return routes.DashboardRoute.to;
        }
        return null;
    }, [isAuthLoading, user, onboardingState, location.pathname]);
    ...
    const commands = useMemo(() => {
        ...
        const items = [];
        const add = (section, label, subtitle, keywords, run) => {
    ...
    const groupedCommands = ['Navigate', 'Search scans', 'Actions', 'Help']
        .map((section) => ({
        section,
      ...
      <Dialog open={isPaletteOpen} onOpenChange={setIsPaletteOpen}>
        <DialogContent className="max-w-2xl" aria-label="Command palette">
          ...
          <div className="space-y-3">
            <input autoFocus value={paletteQuery} onChange={(event) => setPaletteQuery(event.target.value)} placeholder="Search commands and scans" className="w-full rounded-md border border-border/60 bg-background px-3 py-2 text-sm" aria-label="Command palette search"/>
            <div className="max-h-[26rem] overflow-y-auto rounded-md border border-border/60">
              {groupedCommands.length === 0 ? (<p className="p-3 text-sm text-muted-foreground">No matches.</p>) : (<div className="p-2">
                  {groupedCommands.map((group) => (<div key={group.section} className="mb-3 last:mb-0">
                    ...
                    return (<button key={command.id} type="button" role="listitem" className={`w-full rounded-md px-2 py-2 text-left text-sm transition ${active ? 'bg-accent' : 'hover:bg-accent/60'}`} onMouseEnter={() => setPaletteIndex(itemIndex)} onClick={() => {
                            void Promise.resolve(command.run())
                                .catch((error) => {
                                console.error(error);
                                toast({ title: 'Action failed', variant: 'destructive' });
                              ...
                              {command.subtitle && (<p className="text-xs text-muted-foreground mt-0.5">{command.subtitle}</p>)}
                            </button>);
                })}
                      </div>
                    </div>))}
