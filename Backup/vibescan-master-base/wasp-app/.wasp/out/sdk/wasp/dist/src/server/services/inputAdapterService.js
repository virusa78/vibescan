...
import { createGitHubInstallationAccessToken } from './githubAppService';
import { fromCycloneDX, validateCycloneDX, } from '../../ingestion/cyclonedx-contracts.js';
...
export function isJohnnyInstalled() {
    try {
...
export function isSyftInstalled() {
    try {
...
export function resolveTrustedScanInputPath(inputRef) {
...
export function buildCycloneDxSbom(components) {
    return {
        bomFormat: 'CycloneDX',
        specVersion: '1.4',
        version: 1,
 ...
 * Expected: { components: [...] }
 */
export function validateAndExtractSBOM(rawText) {
    ...
    if (ingestion.status !== 'ingested') {
        throw new HttpError(422, 'invalid_sbom', {
            validation_errors: [
                ...
                ...(Array.isArray(ingestion.error.details?.errors) ? ingestion.error.details?.errors : []),
            ],
        });
    }
    const normalized = ingestion.payload.components.map((component) => ({
        name: component.name,
        version: component.version,
        purl: component.purl,
        type: component.type,
...
export function validateGitHubUrl(url) {
    ...
    if (!match) {
        throw new HttpError(422, 'Invalid GitHub URL', { detail: 'Must be in format: https://github.com/owner/repo' });
    }
    return {
        owner: match[1],
        repo: match[2],
...
function normalizeScanInputType(inputType) {
    switch (inputType) {
        case 'github':
        case 'github_app':
        case 'ci_plugin':
...
export async function loadScanArtifacts(inputType, inputRef, options) {
    switch (normalizeScanInputType(inputType)) {
        case 'github': {
            validateGitHubUrl(inputRef);
            const components = await normalizeComponents(await cloneGitHubAndScanWithSBOMGenerator(inputRef, 330000, {
 ...
 */
export function parseSyftOutput(syftJson) {
    let syftData;
    try {
        ...
        throw new HttpError(500, 'Syft parsing failed', { detail: 'Failed to parse Syft SBOM output' });
    }
 ...
 */
export async function normalizeComponents(raw) {
    if (!Array.isArray(raw)) {
        throw new HttpError(422, 'Invalid component format', { detail: 'Components must be an array' });
    }
...
function componentFromPackage(name, version, type = 'library') {
    return {
        name: name.trim(),
...
function parsePackageJsonManifest(filePath) {
    let parsed;
    try {
    ...
    return Object.entries(dependencies).map(([name, version]) => componentFromPackage(name, String(version)));
}
function parseGoModManifest(filePath) {
...
function collectRepoComponents(repoPath) {
    const manifests = [];
 ...
 */
export async function extractZipAndScanWithSBOMGenerator(filePath, timeoutMs = 300000) {
    if (!existsSync(filePath)) {
        throw new HttpError(422, 'ZIP file not found', { detail: `ZIP file does not exist: ${filePath}` });
    }
    ...
    try {
        try {
            ...
            throw new HttpError(422, 'ZIP extraction failed', { detail: `Unable to extract ${filePath}: ${error instanceof Error ? error.message : String(error)}` });
        }
        ...
        if (isJohnnyInstalled()) {
            try {
                ...
                const johnnyOutput = execFileSync('johnny', ['--input', extractDir, '--output-format', 'cyclonedx-json'], { timeout: timeoutMs, encoding: 'utf8' });
        ...
        if (isSyftInstalled()) {
            try {
                const syftOutput = execFileSync('syft', [`dir:${extractDir}`, '-o', 'cyclonedx-json'], { timeout: timeoutMs, encoding: 'utf8' });
 ...
 */
async function parseCycloneDXOutput(jsonOutput) {
 ...
 * @returns Normalized components from scan
 */
export async function cloneGitHubAndScanWithSBOMGenerator(url, timeoutMs = 330000, options) {
    ...
    try {
        try {
            const branchName = options?.githubContext?.branch
                || getBranchNameFromGitRef(options?.githubContext?.ref)
                || undefined;
            const commitSha = options?.githubContext?.commitSha || undefined;
            const authCloneUrl = options?.githubContext?.installationId
                ? `https://x-access-token:${await createGitHubInstallationAccessToken(options.githubContext.installationId)}@github.com/${owner}/${repo}.git`
                : url;
            const cloneArgs = [
                'clone',
                '--depth',
            ...
            execFileSync('git', cloneArgs, { timeout: timeoutMs, stdio: 'pipe' });
            if (commitSha) {
                try {
                    ...
                    console.warn(`[InputAdapter] Failed to checkout commit ${commitSha} for ${url}:`, checkoutError instanceof Error ? checkoutError.message : String(checkoutError));
                }
            }
        }
        catch (error) {
            throw new HttpError(422, 'GitHub repository clone failed', { detail: `Unable to clone ${url}: ${error instanceof Error ? error.message : String(error)}` });
        }
        // Try Johnny first if installed, then Syft
        if (isJohnnyInstalled()) {
            try {
                console.log(`[InputAdapter] Using Johnny to scan repo: ${url}`);
                const johnnyOutput = execFileSync('johnny', ['--input', repoPath, '--output-format', 'cyclonedx-json'], { timeout: timeoutMs, encoding: 'utf8' });
                return parseCycloneDXOutput(johnnyOutput);
            }
            catch (error) {
        ...
        if (isSyftInstalled()) {
            try {
                const syftOutput = execFileSync('syft', [`dir:${repoPath}`, '-o', 'cyclonedx-json'], { timeout: timeoutMs, encoding: 'utf8' });
                return parseCycloneDXOutput(syftOutput);
            }
            catch (error) {
...
//# sourceMappingURL=inputAdapterService.js.map