...
function normalizeScanInputType(inputType) {
    if (inputType === 'github_app' || inputType === 'github')
        return 'github';
    if (inputType === 'sbom_upload' || inputType === 'sbom')
        return 'sbom';
    if (inputType === 'source_zip')
        return 'source_zip';
    return null;
}
async function loadOwnedScans(context, userId, scanIds) {
...
export async function bulkCancelScans(rawArgs, context) {
    const user = ensureUser(context);
    const args = ensureArgsSchemaOrThrowHttpError(bulkInputSchema, rawArgs);
    ...
    for (const [index, scan] of scanRows.entries()) {
        ...
        try {
            ...
            const errorCode = error?.data?.error || 'cancel_failed';
...
export async function bulkRerunScans(rawArgs, context) {
    const user = ensureUser(context);
    const args = ensureArgsSchemaOrThrowHttpError(bulkInputSchema, rawArgs);
    const scanRows = await loadOwnedScans(context, user.id, args.scanIds);
    const results = [];
    for (const [index, scan] of scanRows.entries()) {
        ...
        try {
            ...
            const errorCode = error?.data?.error || 'rerun_failed';
            results.push({ scanId: scan.id, success: false, error: errorCode });
        }
    }
    const succeeded = results.filter((item) => item.success).length;
...
export async function exportScans(rawArgs, context) {
    const user = ensureUser(context);
    const args = ensureArgsSchemaOrThrowHttpError(exportInputSchema, rawArgs);
    const scanRows = await loadOwnedScans(context, user.id, args.scanIds);
    const validRows = scanRows.filter((scan) => scan !== null);
...
//# sourceMappingURL=bulkScanActions.js.map