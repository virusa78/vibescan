...
export function ScanDetailsPage() {
    ...
    const scanDetailsQuery = useQuery(getScanById, { scanId }, {
    ...
    if (!scan && isPolling) {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="flex items-center gap-2 mb-8">
            <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 text-blue-400 hover:text-blue-300">
              <ArrowLeft size={20}/>
              Back to Dashboard
            </button>
          </div>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="border-red-700 bg-red-900/20" data-testid="scan-status-failed">
            ...
            <CardContent className="space-y-4">
              ...
              <button onClick={() => navigate('/dashboard')} className="px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white rounded-md">
                Return to Dashboard
              </button>
            </CardContent>
          </Card>
        ...
        <div className="max-w-4xl mx-auto">
          ...
          <Card className="border-blue-700 bg-blue-900/20" data-testid="scan-status-running">
            ...
            <CardContent className="space-y-6">
              {/* Scan Details */}
              {scan && (<div className="grid grid-cols-2 gap-4 text-sm">
                  <div>
              ...
              <div>
                ...
                <div className="w-full bg-slate-700/50 rounded-full h-3">
                  <div className="bg-blue-500 rounded-full h-3 transition-all duration-300" style={{ width: `${progress}%` }}/>
                </div>
              </div>
          ...
          <div className="grid grid-cols-1 gap-4 md:grid-cols-2 mb-8">
            <Card className="border-slate-700 bg-slate-800/50" data-testid="scanner-summary">
              ...
              <CardContent className="space-y-3">
                {scanResults.length > 0 ? (scanResults.map((result) => (<div key={result.id} className="flex items-center justify-between gap-3 rounded-lg border border-slate-700/70 bg-slate-900/40 px-3 py-2">
                      <div>
                    ...
                    </div>))) : (<p className="text-sm text-slate-400">No scanner results yet.</p>)}
              </CardContent>
            </Card>
            ...
            <Card className="border-slate-700 bg-slate-800/50" data-testid="delta-summary">
              ...
              <CardContent>
                {latestDelta ? (<div className="space-y-3">
                  ...
                  </div>) : (<p className="text-sm text-slate-400">No delta recorded yet.</p>)}
              </CardContent>
            </Card>
          </div>

          {/* Vulnerabilities Table */}
          {report.vulnerabilities && report.vulnerabilities.length > 0 ? (<Card className="border-slate-700 bg-slate-800/50">
              <CardHeader>
              ...
              <CardContent>
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    <tbody>
                      {report.vulnerabilities.map((vuln, idx) => (<React.Fragment key={vuln.id ?? `${vuln.cveId}-${idx}`}>
                          ...
                          {(vuln.description || vuln.filePath) && (<tr className="border-b border-slate-700 bg-slate-800/40">
            ...
            </Card>) : (<Card className="border-slate-700 bg-slate-800/50">
              <CardContent className="pt-6">
    ...
    return (<div className="min-h-screen bg-gradient-to-br from-slate-900 to-slate-800 p-8">
      <div className="max-w-4xl mx-auto">
        <button onClick={() => navigate('/dashboard')} className="flex items-center gap-2 text-blue-400 hover:text-blue-300 mb-8">
          <ArrowLeft size={20}/>
          Back to Dashboard
        </button>
...
//# sourceMappingURL=ScanDetailsPage.jsx.map