...
export function normalizeWaspRoutePath(path) {
    return path.replace(pathTokenRegex, '{$1}');
}
export function parseV1RoutesFromMainWasp(mainWaspSource) {
...
function hasHttpClassResponse(responses, clazz) {
    if (!responses) {
        return false;
    }
    return Object.keys(responses).some((code) => code.startsWith(clazz));
}
function hasErrorSchemaRef(response) {
    if (!response) {
        return false;
    }
...
//# sourceMappingURL=openapiContractPolicy.js.map