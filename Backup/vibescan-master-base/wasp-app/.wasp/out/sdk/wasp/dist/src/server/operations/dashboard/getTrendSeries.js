...
function addGranularity(value, granularity) {
...
function normalizeBucketStart(value, granularity) {
    return granularity === 'week' ? startOfUtcWeek(value) : startOfUtcDay(value);
}
function getDateRangeStart(timeRange, now, earliestScanCreatedAt) {
    if (timeRange === '7d') {
...
export async function getTrendSeries(rawArgs, context) {
    const user = await requireWorkspaceScopedUser(context.user);
    const args = ensureArgsSchemaOrThrowHttpError(getTrendSeriesInputSchema, rawArgs);
    const now = new Date();
    const granularity = args.granularity ?? (args.time_range === 'all' ? 'week' : 'day');
    ...
    for (let cursor = new Date(normalizedRangeStart); cursor.getTime() <= normalizedRangeEnd.getTime(); cursor = addGranularity(cursor, granularity)) {
    ...
    const buckets = [...bucketMap.values()].sort((a, b) => new Date(a.bucket_start).getTime() - new Date(b.bucket_start).getTime());
    const totals = buckets.reduce((acc, bucket) => ({
        ...
        findings_by_source: Object.entries(bucket.findings_by_source).reduce((sourceAccumulator, [source, count]) => {
            sourceAccumulator[source] = (acc.findings_by_source[source] ?? 0) + count;
            return sourceAccumulator;
        }, { ...acc.findings_by_source }),
    }), { scans: 0, findings: 0, delta: 0, findings_by_source: {} });
    return {
...
//# sourceMappingURL=getTrendSeries.js.map