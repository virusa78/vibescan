...
async function readPreferencesForUser(userId: string): Promise<any> {
...
async function writePreferencesForUser(userId: string, preferences: UserUiPreferences): Promise<any> {
...
export async function listScanSavedViews(
  _rawArgs: unknown,
  context: any,
): Promise<any> {
...
export async function createScanSavedView(
  rawArgs: unknown,
  context: any,
): Promise<any> {
  const user = ensureUser(context);
...
export async function updateScanSavedView(
  rawArgs: unknown,
  context: any,
): Promise<any> {
  const user = ensureUser(context);
  ...
  const preferences = await readPreferencesForUser(user.id);
  const existing = preferences.scanSavedViews.find((view: any) => view.id === args.viewId);
  ...
  const nextPreferences: UserUiPreferences = {
    ...preferences,
    scanSavedViews: preferences.scanSavedViews.map((view: any) => (view.id === existing.id ? nextView : view)),
  };
...
export async function deleteScanSavedView(
  rawArgs: unknown,
  context: any,
): Promise<any> {
  const user = ensureUser(context);
  ...
  const nextViews = preferences.scanSavedViews.filter((view: any) => view.id !== args.viewId);
