...
function getRuntimeMode(): 'auto' | 'docker' | 'local' {
...
function buildDockerMount(targetPath: string): { mountSource: string; containerTarget: string } {
...
export function buildDockerScannerArgs(targetPath: string, image: string, toolArgs: string[]): string[] {
...
function buildPlan(plan: ScannerExecutionPlan): { dockerCommand: string; dockerArgs: string[]; localCommand: string; localArgs: string[] } {
  return {
  ...
  if (runtimeMode !== 'local') {
    try {
      ...
      if (!isMissingExecutable(error, 'docker')) {
        console.warn(
          `[ScannerRuntime] Docker execution failed for ${plan.tool}, falling back to local CLI: ${error instanceof Error ? error.message : String(error)}`,
        );
      }
    }
  }
