...
const CANONICAL_EVENT_CATEGORY_MAP = new Map(
  canonicalEventRegistry.map((entry) => [entry.type, entry.category]),
);

const CANONICAL_TO_LEGACY_WEBHOOK_EVENT_MAP: Record<string, string | null> = {
  'scan.completed': 'scan_complete',
  'scan.failed': 'scan_failed',
  'report.generated': 'report_ready',
};

async function validateDestinationConfig(
  destinationType: CreateEventSubscriptionInput['destination_type'],
...
async function assertNoLegacyWebhookOwnershipConflict(args: {
  ...
  if (conflictingLegacyWebhook) {
    throw new HttpError(
      409,
      'This destination is already owned by the legacy webhook subsystem for overlapping events. Migrate or disable the legacy webhook first.',
    );
  }
}

function validateCanonicalEventSelections(eventTypes: string[], categories: string[]) {
  for (const eventType of eventTypes) {
    ...
    if (!categories.includes(actualCategory)) {
      throw new HttpError(
        400,
        `Event type ${eventType} belongs to category ${actualCategory}, but that category is not included in the subscription`,
      );
    }
  }
}

export async function createEventSubscription(rawArgs: unknown, context: any): Promise<any> {
  ...
  const signingSecretEncrypted =
    args.destination_type === 'zoho_crm'
      ? null
