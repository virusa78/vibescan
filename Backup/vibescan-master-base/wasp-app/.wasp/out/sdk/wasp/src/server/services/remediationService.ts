...
type RemediationTransaction = typeof prisma;
...
async function resolvePolicyForUser(tx: RemediationTransaction, userId: string) {
...
function getMonthlyPromptLimitFromSnapshot(snapshot: unknown): number {
  if (snapshot && typeof snapshot === 'object' && 'monthlyRemediationPromptLimit' in snapshot) {
    return Number((snapshot as { monthlyRemediationPromptLimit?: unknown }).monthlyRemediationPromptLimit ?? 0);
  }
  return 0;
}
...
export async function generateCveRemediation(
  input: GenerateCveRemediationInput,
): Promise<GenerateCveRemediationResponse> {
  ...
  return prisma.$transaction(async (tx) => {
    ...
    if (existingUsage) {
      if (existingUsage.status === 'rejected_quota') {
        throw new HttpError(429, 'Remediation prompt quota exceeded', {
          ...
          quota_limit:
            getMonthlyPromptLimitFromSnapshot(existingUsage.policySnapshot),
          quota_remaining: 0,
        });
      }
    ...
    const effectivePolicy = await resolvePolicyForUser(tx as any, user.id);
      ...
      await tx.remediationPromptUsage.create({
        data: {
          ...
          regionAtCall: effectivePolicy.regionCode,
           policySnapshot: effectivePolicy as unknown as Prisma.InputJsonValue,
          requestKey: request.requestKey,
          promptType,
    ...
    const prompt = await tx.aiFixPrompt.create({
      data: {
        ...
        responsePayload: draft.responsePayload as unknown as Prisma.InputJsonValue,
        status: 'generated',
      },
    });
