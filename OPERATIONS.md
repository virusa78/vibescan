...
This document contains a large amount of historical operation detail and is not yet fully rewritten to the current workspace/onboarding/GitHub product model.

## Current Accuracy Note

Use these files as the current source of truth before trusting examples below:

- `wasp-app/main.wasp`
- `wasp-app/src/server/swagger/v1EndpointManifest.ts`
- `wasp-app/src/server/swagger/schemas.ts`
- `plan.md`

## Current Product Additions Not Fully Reflected Below

- active workspace context and workspace switching
- onboarding state and onboarding completion endpoints
- GitHub installation listing/linking/settings endpoints
- GitHub App webhook ingestion and GitHub Check Run lifecycle wiring

The older sections below are still useful as legacy reference, but they should not be treated as the authoritative description of the newest workspace/onboarding/GitHub surfaces.
