...
function getSpecV1Routes(spec: OpenApiDocument): RouteSignature[] {
  ...
  for (const [pathName, pathItemRaw] of Object.entries(paths)) {
    ...
    for (const method of HTTP_METHODS) {
      const maybeOperation = (pathItem as any)[method] as Record<string, any> | undefined;
      if (maybeOperation) {
...
function hasHttpClassResponse(responses: Record<string, any> | undefined, clazz: '2' | '4' | '5'): boolean {
  if (!responses) {
    return false;
  }
...
function hasErrorSchemaRef(response: Record<string, any>): boolean {
...
export function validateV1OpenApiContract(input: {
  ...
  for (const route of mainRoutes) {
    ...
    if (!specRouteKeySet.has(key)) {
      violations.push({
        ...
        route: key,
        detail: 'Route is present in main.wasp but missing in OpenAPI spec.',
      });
    }
  ...
  for (const route of specRoutes) {
    const key = routeKey(route);
    if (!mainRouteKeySet.has(key)) {
      violations.push({
        code: 'route_extra_in_spec',
        route: key,
        detail: 'Route is present in OpenAPI spec but not declared as /api/v1 route in main.wasp.',
      });
    }
  }
  ...
  for (const route of mainRoutes) {
    const key = routeKey(route);
    const pathItem = input.spec.paths?.[route.path] as Record<string, any> | undefined;
    const operation = pathItem ? ((pathItem as any)[route.method] as Record<string, any> | undefined) : undefined;
    ...
    const has4xxErrorRef = Object.entries(operation.responses ?? {}).some(([code, response]) => {
      return code.startsWith('4') && hasErrorSchemaRef(response as any);
    });
    const has5xxErrorRef = Object.entries(operation.responses ?? {}).some(([code, response]) => {
      return code.startsWith('5') && hasErrorSchemaRef(response as any);
    });
