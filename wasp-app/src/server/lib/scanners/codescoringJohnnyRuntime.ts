...
type JohnnyRuntimeMode = 'auto' | 'ssh' | 'api' | 'mock';
...
function getRuntimeMode(): JohnnyRuntimeMode {
  const value = process.env.CODESCORING_RUNTIME?.trim().toLowerCase();
  if (value === 'ssh' || value === 'api' || value === 'mock') {
    return value;
  }
  return 'auto';
}
