...
export async function enterpriseScannerWorker(job: Job<ScanJob>) {
  ...
  try {
    ...
    if (startedScan.count === 0) {
      console.log(`[Enterprise Scanner] Scan ${scanId} is no longer active, skipping`);
      return {
        status: 'skipped',
        findingsCount: 0,
        scanResultId: null,
    ...
    const batchSize = 100;
    for (let i = 0; i < normalizedFindings.length; i += batchSize) {
      const batch = normalizedFindings.slice(i, i + batchSize);

      const upsertPromises = batch.map((finding) => {
