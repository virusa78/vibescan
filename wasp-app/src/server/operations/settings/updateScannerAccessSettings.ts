...
import type { ScannerAccessResponse } from './getScannerAccessSettings';
...
export async function updateScannerAccessSettings(
  rawArgs: unknown,
  context: any
): Promise<any> {
  if (!context.user) {
  ...
  const args = ensureArgsSchemaOrThrowHttpError(
    updateScannerAccessSettingsSchema,
    rawArgs
  );
  ...
  const updatedEncryptedKey =
    snykApiKeyEncrypted !== undefined ? snykApiKeyEncrypted : user.snykApiKeyEncrypted;
