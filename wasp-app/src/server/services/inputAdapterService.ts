...
function getBranchNameFromGitRef(ref: string | null | undefined): string | null {
  if (!ref) {
    return null;
  }
  ...
  switch (normalizeScanInputType(inputType)) {
    case 'github': {
      ...
      const components = await normalizeComponents(
        await cloneGitHubAndScanWithSBOMGenerator(inputRef, 330000, {
...
async function parseCycloneDXOutput(jsonOutput: string): Promise<NormalizedComponent[]> {
  ...
  const components = Array.isArray(parsed?.components)
    ? parsed.components
        .map((component: { name?: unknown; version?: unknown; purl?: unknown; type?: unknown }) => {
    ...
    try {
      ...
      if (commitSha) {
        try {
          ...
          console.warn(
            `[InputAdapter] Failed to checkout commit ${commitSha} for ${url}:`,
