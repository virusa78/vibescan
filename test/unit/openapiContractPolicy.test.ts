...
describe('openapi v1 contract policy', () => {
  ...
  it('falls back when primary scan returns empty v1 paths', async () => {
    const calls: string[][] = [];
    const generator = jest.fn((options: { apis: string[]; definition: Record<string, unknown> }) => {
    ...
    expect((spec as { __generationSource?: string }).__generationSource).toBe('absolute-fallback-glob');
    expect(spec.paths?.['/api/v1/scans']?.get).toBeDefined();
  });

  it('fails fast with diagnostics when both scans are empty', async () => {
    const generator = jest.fn((options: { definition: Record<string, unknown> }) => ({
      ...options.definition,
      paths: {},
    }));
  ...
  it('matches main.wasp /api/v1 routes against generated OpenAPI contract', async () => {
    ...
    expect(report.routeCountMainWasp).toBe(routes.length);
    expect(report.routeCountSpec).toBeGreaterThanOrEqual(report.routeCountMainWasp);
  });
});
