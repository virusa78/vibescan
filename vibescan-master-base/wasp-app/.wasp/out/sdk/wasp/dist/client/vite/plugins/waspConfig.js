...
function useUserValue(userValue, defaultValue) {
    return userValue ?? defaultValue;
}
function throwIfOverridingForcedOptions(config) {
    const conflicts = [];
    for (const [path, forcedValue] of Object.entries(forcedOptions)) {
        ...
        if (userValue !== undefined && userValue !== forcedValue) {
            conflicts.push(`  - "${path}" is set to ${JSON.stringify(userValue)}, but Wasp requires ${JSON.stringify(forcedValue)}`);
        }
    }
    if (conflicts.length > 0) {
        throw new Error(`Your vite.config.ts sets options that Wasp controls:\n${conflicts.join('\n')}\n\nRemove these from your Vite config, Wasp sets them automatically.`);
    }
}
function getByPath(obj, path) {
    return path.split('.').reduce((node, segment) => node?.[segment], obj);
}
//# sourceMappingURL=waspConfig.js.map