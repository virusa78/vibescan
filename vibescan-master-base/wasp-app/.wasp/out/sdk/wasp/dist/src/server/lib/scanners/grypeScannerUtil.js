...
function isGrypeMissingError(error) {
 ...
 */
function generateCycloneDxSbom(components) {
    const sbom = {
        bomFormat: 'CycloneDX',
        specVersion: '1.4',
        version: 1,
 ...
 */
export async function executeGrypeCli(sbomPath, timeoutMs = 300000) {
 ...
 */
export function parseGrypOutput(rawOutput) {
 ...
 */
export async function scanWithGrype(components, scanId, timeoutMs = 300000) {
    const result = await scanWithGrypeDetailed(components, scanId, timeoutMs);
    return result.findings;
}
export async function scanWithGrypeDetailed(components, scanId, timeoutMs = 300000) {
    let sbomPath = null;
    try {
        if (!isGrypInstalled()) {
 ...
 */
export function isGrypInstalled() {
    try {
...
//# sourceMappingURL=grypeScannerUtil.js.map