...
import { createWebhook, deleteWebhook, listWebhookDeliveries, listWebhooks, retryWebhookDelivery, testWebhookDelivery, updateWebhook, } from 'wasp/client/operations';
...
export default function WebhooksPage() {
    ...
    const selectedDelivery = useMemo(() => deliveries.find((delivery) => delivery.id === selectedDeliveryId) ?? deliveries[0] ?? null, [deliveries, selectedDeliveryId]);
    const loadWebhooks = async () => {
        await run(async () => {
            const data = await listWebhooks(undefined);
    ...
    const loadDeliveries = async (webhookId, cursor) => {
        const data = await listWebhookDeliveries({
            webhookId,
            limit: 100,
            ...(cursor ? { cursor } : {}),
        });
        const nextItems = Array.isArray(data.deliveries) ? data.deliveries : [];
    ...
    const openWebhookDrawer = async (webhook) => {
    ...
    const handleToggleActive = async (id) => {
        ...
        await run(async () => {
            ...
            setWebhooks((previous) => previous.map((webhook) => (webhook.id === id ? { ...webhook, enabled: !webhook.enabled } : webhook)));
            if (selectedWebhook?.id === id) {
    ...
    return (<div className="p-8 lg:p-10">
      ...
      {error && (<div className="mb-6 rounded-md border border-red-500/40 bg-red-500/10 p-3 text-sm text-red-600">{error}</div>)}

      {isAddingNew && (<Card className="mb-8 border-primary/50 bg-primary/5">
          <CardHeader>
          ...
          <CardContent className="space-y-4">
            <div>
              ...
              <input type="url" placeholder="https://your-domain.com/webhooks/vibescan" value={newWebhookUrl} onChange={(event) => setNewWebhookUrl(event.target.value)} className="w-full px-3 py-2 border border-border rounded-md bg-background text-foreground placeholder-muted-foreground focus:outline-none focus:ring-2 focus:ring-primary"/>
            </div>
      ...
      {isLoading ? (<div className="space-y-4">
          ...
          <WebhookCardSkeleton />
          <WebhookCardSkeleton />
        </div>) : webhooks.length === 0 ? (<Card className="border-dashed border-border/50 bg-card/50 backdrop-blur-sm">
          <CardHeader>
          ...
          {webhooks.map((webhook) => (<Card key={webhook.id} className="border-border/50 bg-card/50 backdrop-blur-sm cursor-pointer" onClick={() => void openWebhookDrawer(webhook)}>
              <CardHeader className="pb-3">
                <div className="flex items-start justify-between gap-4">
                  <div className="flex-1">
                    <div className="flex items-center gap-2">
                      <code className="text-sm font-mono text-foreground break-all">{webhook.url}</code>
                      <div className={`px-2 py-1 rounded-md text-xs font-semibold ${webhook.enabled ? 'bg-green-500/20 text-green-600' : 'bg-gray-500/20 text-gray-600'}`}>
                  ...
                  <div className="flex items-center gap-2" onClick={(event) => event.stopPropagation()}>
                    ...
                    <Button variant="outline" size="sm" className="border-red-500/30 hover:bg-red-500/10 hover:text-red-600" onClick={() => void handleDeleteWebhook(webhook.id)} aria-label={`Delete webhook ${webhook.url}`}>
      ...
      {selectedWebhook && (<Card className="mt-8 border-border/50 bg-card/50 backdrop-blur-sm">
          <CardHeader className="space-y-3">
            ...
            <div className="flex flex-wrap items-center gap-2">
              {[
                ...
                ['payloads', 'Payloads'],
                ['settings', 'Settings'],
            ].map(([tab, label]) => (<button key={tab} type="button" className={`rounded-full border px-3 py-1 text-xs ${drawerTab === tab
                    ? 'border-primary bg-primary text-primary-foreground'
                    : 'border-border/60 text-muted-foreground'}`} onClick={() => setDrawerTab(tab)}>
                  {label}
                </button>))}
            </div>
          </CardHeader>
          <CardContent>
            {drawerTab === 'overview' && (<div className="grid grid-cols-1 md:grid-cols-3 gap-4 text-sm">
            ...
            {drawerTab === 'deliveries' && (<div className="space-y-3">
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
                    ...
                    return (<tr key={delivery.id} className={`border-b border-border/20 cursor-pointer ${selectedDelivery?.id === delivery.id ? 'bg-accent/20' : ''}`} onClick={() => setSelectedDeliveryId(delivery.id)}>
                            ...
                            <td className="py-2 px-2">
                              <Button disabled={!canRetry} onClick={(event) => {
                ...
                {deliveryCursor && (<Button onClick={() => void loadDeliveries(selectedWebhook.id, deliveryCursor)}>Load more</Button>)}
              </div>)}

            {drawerTab === 'payloads' && (<div className="space-y-3">
                {!selectedDelivery ? (<p className="text-sm text-muted-foreground">Select a delivery from the Deliveries tab.</p>) : (<>
                    <div className="rounded border border-border/50 p-3">
            ...
            {drawerTab === 'settings' && (<div className="space-y-4">
                ...
                <div className="flex items-center gap-2">
                  ...
                  <Button variant="outline" className="border-red-500/30 hover:bg-red-500/10 hover:text-red-600" onClick={() => void handleDeleteWebhook(selectedWebhook.id)}>
                    Delete webhook
                  </Button>
                </div>
              </div>)}
...
//# sourceMappingURL=WebhooksPage.jsx.map