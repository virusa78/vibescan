...
import { scannerWorkerDefinitions } from './scannerWorkerRouting.js';
const workerRole = getWorkerRole();
const redisConfig = getRedisConnectionConfig();
async function run() {
    const definition = scannerWorkerDefinitions[workerRole];
    if (!definition) {
        throw new Error(`Unknown WORKER_ROLE "${process.env.WORKER_ROLE}". Expected one of: ${Object.keys(scannerWorkerDefinitions).join(', ')}`);
    }
    ...
    console.log(`[${definition.label}] Worker started for queue ${definition.queueName} with concurrency ${definition.concurrency}`);
}
run().catch((error) => {
    console.error('[Worker Entrypoint] Failed to start worker:', error);
    process.exit(1);
});
//# sourceMappingURL=runQueueWorker.js.map