...
function buildDecisionSummary(snapshot) {
    return toJsonRecord({
...
export function resolveCycloneDxRolloutStage() {
...
export function buildCycloneDxRolloutSnapshotRecord(options) {
    const stage = options.stage || options.ingestionMeta.gate.canaryDecision.stage;
...
function snapshotHasAnyBlockers(snapshots, driftThreshold) {
    return snapshots.some((snapshot) => {
        return (snapshot.decisionStatus === 'rollback_required' ||
...
export function buildCycloneDxRolloutMarkdown(evidence) {
    const stageRows = Object.entries(evidence.summary.stageBreakdown)
        .map(([stage, count]) => `- ${stage}: ${count}`)
        .join('\n');
    const snapshotRows = evidence.snapshots
        .map((snapshot) => `- ${snapshot.scannerId} / ${snapshot.scanResultId} / ${snapshot.rolloutStage} / ${snapshot.decisionStatus} / ${snapshot.gateStatus}`)
        .join('\n');
    return [
...
export async function persistCycloneDxRolloutDecision(options) {
    ...
    const progressStatus = options.decision.status === 'rollback_required'
        ? 'rollback_active'
...
export async function collectCycloneDxRolloutWindow(options) {
    ...
    return snapshots.map((snapshot) => normalizeSnapshot({
        id: snapshot.id,
        ...
        ingestionMeta: snapshot.ingestionMeta,
    }));
}
//# sourceMappingURL=cyclonedxRolloutGovernance.js.map