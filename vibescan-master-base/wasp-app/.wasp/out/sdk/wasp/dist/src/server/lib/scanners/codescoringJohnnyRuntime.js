...
import { shellQuote } from './remoteSsh.js';
function buildCycloneDxSbom(components) {
    return {
        bomFormat: 'CycloneDX',
        specVersion: '1.6',
        version: 1,
...
function getJohnnyRuntimeConfig() {
    ...
    return {
        ...
        commandTemplate: process.env.CODESCORING_JOHNNY_COMMAND?.trim()
...
function buildRemoteShellCommand(commandTemplate, remoteBomPath) {
...
function buildRawSshArgs(config, remoteShellCommand) {
...
function normalizeSuccessfulOutput(stdout, status) {
...
function resolveJohnnyResult(result) {
...
export function buildJohnnySshArgs(config, remoteShellCommand) {
    return buildRawSshArgs(config, remoteShellCommand);
}
export function isJohnnyRemoteConfigured() {
    return getJohnnyRuntimeConfig() !== null;
}
export function runJohnnyScanViaSsh(components, scanId, timeoutMs, executor = defaultExecutor) {
...
//# sourceMappingURL=codescoringJohnnyRuntime.js.map