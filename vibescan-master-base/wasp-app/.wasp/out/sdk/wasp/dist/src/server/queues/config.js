...
import { QUEUE_NAMES, } from './jobContract.js';
...
export const eventDeliveryQueue = new Queue(QUEUE_NAMES.EVENT_DELIVERY, {
    connection: redisConfig,
    defaultJobOptions: {
        attempts: 5,
        backoff: {
...
function logCompletedJob(label, job) {
    console.log(`[${label}] Job ${job?.id ?? 'unknown'} completed`);
}
function logFailedJob(label, job, error) {
...
export async function initializeWorkers() {
    ...
    try {
        ...
        freeWorker = new Worker(scannerWorkerDefinitions.free.queueName, scannerWorkerDefinitions.free.processor, {
            connection: redisConfig,
        ...
        enterpriseWorker = new Worker(scannerWorkerDefinitions.enterprise.queueName, scannerWorkerDefinitions.enterprise.processor, {
            connection: redisConfig,
