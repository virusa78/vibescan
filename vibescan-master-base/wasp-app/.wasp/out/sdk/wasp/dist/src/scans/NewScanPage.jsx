...
export default function NewScanPage() {
    ...
    const { data: recentScans, isLoading: isRecentLoading, error: recentError, refetch, } = useQuery(getScans);
    const normalizedScans = useMemo(() => recentScans ?? [], [recentScans]);
    const activeType = useMemo(() => inputTypeOptions.find((option) => option.value === inputType) ?? inputTypeOptions[0], [inputType]);
    useEffect(() => {
    ...
    const onSubmit = async (event) => {
        ...
        await run(async () => {
            ...
            setInputRef("");
            await refetch();
            navigate(routes.ScanDetailsRoute.build({ scanId: createdScan.id }));
    ...
    return (<div className="py-10 lg:mt-10">
      <div className="mx-auto max-w-5xl px-6 lg:px-8">
        ...
        {source === "onboarding" && (<Alert className="mt-8">
            <AlertDescription>
        ...
        <Card className="border-border/70 mt-8 bg-card/90 shadow-sm">
          ...
          <CardContent className="space-y-6">
            ...
            return (<button key={option.value} type="button" onClick={() => setInputType(option.value)} className={`rounded-xl border p-4 text-left transition ${isActive
            ...
            <form onSubmit={onSubmit} className="space-y-4">
              ...
              <div className="space-y-2">
                ...
                <Input id="inputRef" value={inputRef} onChange={(e) => setInputRef(e.target.value)} placeholder={activeType.placeholder}/>
              ...
              <div className="flex flex-col gap-3 sm:flex-row">
                ...
                <Button type="button" variant="outline" onClick={() => navigate(routes.OnboardingRoute.to)}>
                  Open guided setup
                </Button>
              </div>
            </form>
        ...
        <Card className="border-border/70 mt-8 bg-card/90 shadow-sm">
          ...
          <CardContent>
            {isRecentLoading && (<div className="space-y-3">
                <Skeleton className="h-4 w-48"/>
                <Skeleton className="h-10 w-full"/>
                <Skeleton className="h-10 w-full"/>
                <Skeleton className="h-10 w-full"/>
              </div>)}
            {recentError && !isRecentLoading && (<Alert variant="destructive">
                <AlertDescription>
            ...
            {!isRecentLoading && !recentError && normalizedScans.length === 0 && (<div className="text-muted-foreground text-sm">
                Your workspace has no scans yet. The first one will appear here immediately after submission.
              </div>)}
            {!isRecentLoading && !recentError && normalizedScans.length > 0 && (<div className="overflow-x-auto">
                <table className="w-full text-sm">
                  ...
                  <tbody>
                    {normalizedScans.map((scan) => (<tr key={scan.id} className="border-border/50 border-b">
                        ...
                        <td className="py-3 text-right">
                          <WaspRouterLink to={routes.ScanDetailsRoute.to} params={{ scanId: scan.id }} className="text-primary text-xs font-medium underline underline-offset-2">
                            View details
                          </WaspRouterLink>
                        </td>
                      </tr>))}
...
//# sourceMappingURL=NewScanPage.jsx.map