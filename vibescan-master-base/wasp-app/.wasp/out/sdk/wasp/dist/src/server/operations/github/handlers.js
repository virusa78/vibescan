...
import { getMappedGithubInstallationByInstallationId, removeGithubInstallation, syncGithubInstallation, } from '../../services/githubInstallationService';
...
import { isForkPullRequest, isRepositoryAllowedByInstallation, isTargetBranchAllowedByInstallation, } from '../../services/githubWebhookFiltering';
...
function getRawBodyBuffer(body) {
...
async function resolveWorkspaceActorUserId(workspaceId) {
...
async function hasExistingDeliveryScan(workspaceId, deliveryId) {
...
async function enqueueGithubWebhookScan(input) {
    ...
    const result = await submitGitHubScan(actorUserId, input.installation.workspaceId, input.repositoryHtmlUrl, githubContext);
...
async function handlePushWebhook(payload, installation, deliveryId) {
    ...
    const queued = await enqueueGithubWebhookScan({
        ...
        repositoryFullName,
        repositoryHtmlUrl,
        repositoryId: typeof repository?.id === 'number' ? repository.id : undefined,
        repositoryPrivate: typeof repository?.private === 'boolean' ? repository.private : undefined,
...
async function handlePullRequestWebhook(payload, installation, deliveryId) {
    ...
    if (isForkPullRequest(typeof headRepository?.full_name === 'string' ? headRepository.full_name : null, repositoryFullName)) {
...
export async function githubWebhookApiHandler(request, response, _context) {
    try {
        ...
        if (eventName === 'push' || eventName === 'pull_request') {
            const installation = await getMappedGithubInstallationByInstallationId(installationId);
            if (!installation) {
                await syncGithubInstallation(installationId);
            }
            const resolvedInstallation = await getMappedGithubInstallationByInstallationId(installationId);
