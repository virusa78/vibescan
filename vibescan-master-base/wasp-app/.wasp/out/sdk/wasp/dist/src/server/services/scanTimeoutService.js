...
async function cancelExpiredScan(scanId, userId, timeoutMs) {
...
export async function sweepExpiredScans(now = new Date()) {
    if (sweepRunning) {
        return 0;
    }
    sweepRunning = true;
    try {
        ...
        if (cancelledCount > 0) {
            console.log(`[ScanTimeout] Cancelled ${cancelledCount} stale scan(s) older than ${timeoutMs}ms`);
        }
...
export function startScanTimeoutSweeper() {
    if (sweepTimer) {
        return;
    }
...
export async function stopScanTimeoutSweeper() {
    if (!sweepTimer) {
        return;
    }
...
//# sourceMappingURL=scanTimeoutService.js.map