...
export async function emitWebhookEvent(event) {
    try {
        ...
        console.log(`[WebhookEmitter] Found ${webhooks.length} webhooks for event: ${eventType}, scan: ${scanId}`);
        ...
        for (const webhook of webhooks) {
            try {
                ...
                if (existingDelivery) {
                    console.log(`[WebhookEmitter] Skipping duplicate delivery: webhook ${webhook.id}, scan ${scanId}`);
                    continue;
                }
                ...
                await webhookDeliveryQueue.add(`delivery-${delivery.id}`, jobData, {
                ...
                console.log(`[WebhookEmitter] Enqueued delivery job for webhook: ${webhook.id}, delivery: ${delivery.id}`);
            }
            catch (error) {
                console.error(`[WebhookEmitter] Failed to enqueue webhook ${webhook.id}:`, error instanceof Error ? error.message : String(error));
                // Continue with next webhook instead of failing entire event
            }
        }
    }
    catch (error) {
        console.error(`[WebhookEmitter] Failed to emit webhook event:`, error instanceof Error ? error.message : String(error));
 ...
 */
export function buildWebhookPayload(eventType, scanId, userId, scanData) {
    const basePayload = {
        event: eventType,
...
//# sourceMappingURL=webhookEventEmitter.js.map