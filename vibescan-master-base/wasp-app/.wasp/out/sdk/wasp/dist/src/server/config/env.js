...
const workerRoleSchema = z.preprocess((value) => typeof value === 'string' ? value.trim().toLowerCase() : value, z.enum(['free', 'enterprise']).default('free'));
const embedWorkersSchema = z.preprocess((value) => typeof value === 'string' ? value.trim().toLowerCase() : value, z
    .enum(['true', 'false'])
    .default('true')
...
export function getEncryptionKeyHex(env = process.env) {
    return encryptionKeySchema.parse(env.ENCRYPTION_KEY);
}
export function getEncryptionKeyBuffer(env = process.env) {
    return Buffer.from(getEncryptionKeyHex(env), 'hex');
}
export function getNodeEnv(env = process.env) {
...
export function shouldEmbedWorkersFromEnv(env = process.env) {
    return embedWorkersSchema.parse(env.VIBESCAN_EMBED_WORKERS);
}
export function getWorkerRole(env = process.env) {
    return workerRoleSchema.parse(env.WORKER_ROLE);
}
export function getRedisHost(env = process.env) {
    return optionalStringSchema.parse(env.REDIS_HOST) || '127.0.0.1';
}
export function getRedisPort(env = process.env) {
