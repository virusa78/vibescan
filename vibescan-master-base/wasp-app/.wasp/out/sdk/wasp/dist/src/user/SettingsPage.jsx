...
import { useQuery, updateUserSettings, getNotificationSettings, getScannerAccessSettings, linkGithubInstallation, getGithubAppSetup, listGithubInstallations, updateNotificationSettings, updateScannerAccessSettings, updateGithubInstallationSettings, } from "wasp/client/operations";
...
export default function SettingsPage() {
    ...
    const { isLoading: isProfileLoading, error: profileError, run: runProfile, } = useAsyncState();
    const { isLoading: isNotificationLoading, error: notificationError, run: runNotification, } = useAsyncState();
    const { isLoading: isScannerAccessLoading, error: scannerAccessError, run: runScannerAccess, } = useAsyncState();
    const { isLoading: isGitHubLoading, error: githubError, run: runGitHub, } = useAsyncState();
    ...
    const [scannerAccessSuccessMessage, setScannerAccessSuccessMessage] = useState(null);
    ...
    const [snykApiKeyPreview, setSnykApiKeyPreview] = useState(null);
    const [snykEnabled, setSnykEnabled] = useState(false);
    const [snykReady, setSnykReady] = useState(false);
    const [snykReadyReason, setSnykReadyReason] = useState(null);
    const [snykCredentialSource, setSnykCredentialSource] = useState(null);
    ...
    const [targetBranchesByInstallation, setTargetBranchesByInstallation] = useState({});
    const [pushTriggerByInstallation, setPushTriggerByInstallation] = useState({});
    const [prTriggerByInstallation, setPrTriggerByInstallation] = useState({});
    const [severityByInstallation, setSeverityByInstallation] = useState({});
    const { data: githubAppSetupQuery, } = useQuery(getGithubAppSetup);
    const { data: githubInstallationsQuery, isLoading: isGitHubInstallationsLoading, refetch: refetchGithubInstallations, } = useQuery(listGithubInstallations);
    const githubAppSetup = githubAppSetupQuery ?? null;
    const githubInstallations = useMemo(() => (githubInstallationsQuery?.installations ?? []), [githubInstallationsQuery]);
    useEffect(() => {
        if (user) {
            setDisplayName(user.displayName ?? user.username ?? "");
            setTimezone(user.timezone ?? "");
            setLanguage(user.language ?? "en");
            setRegion(user.region === "IN" || user.region === "PK" ? user.region : "OTHER");
        }
    }, [user]);
    const loadNotificationSettings = useCallback(async (targetProjectKey) => {
        const normalizedProjectKey = normalizeProjectKey(targetProjectKey);
        await runNotification(async () => {
            const data = await getNotificationSettings({ project_key: normalizedProjectKey });
    ...
    const loadScannerAccessSettings = useCallback(async () => {
        await runScannerAccess(async () => {
            const data = await getScannerAccessSettings({});
            setSnykApiKey("");
    ...
    useEffect(() => {
        ...
        setReposByInstallation(Object.fromEntries(githubInstallations.map((installation) => [installation.id, installation.repos_scope])));
        setTargetBranchesByInstallation(Object.fromEntries(githubInstallations.map((installation) => [installation.id, installation.target_branches.join(", ")])));
        setPushTriggerByInstallation(Object.fromEntries(githubInstallations.map((installation) => [installation.id, installation.trigger_on_push])));
        setPrTriggerByInstallation(Object.fromEntries(githubInstallations.map((installation) => [installation.id, installation.trigger_on_pr])));
        setSeverityByInstallation(Object.fromEntries(githubInstallations.map((installation) => [installation.id, installation.fail_pr_on_severity])));
    }, [githubInstallations]);
    ...
    const onLoadNotificationSettings = async (event) => {
        event.preventDefault();
        setNotificationSuccessMessage(null);
        await loadNotificationSettings(projectKey);
    };
    const onSaveNotificationSettings = async (event) => {
        event.preventDefault();
        setNotificationSuccessMessage(null);
        ...
        await runNotification(async () => {
            ...
            setSmsEnabled(data.sms_enabled ?? false);
            setNotificationSuccessMessage(`Notification settings saved for project "${data.project_key ?? normalizedProjectKey}".`);
    ...
    const onSaveScannerAccessSettings = async (event) => {
        event.preventDefault();
        setScannerAccessSuccessMessage(null);
        await runScannerAccess(async () => {
    ...
    const onSaveGithubInstallationSettings = async (installation) => {
        setGithubSuccessMessage(null);
        await runGitHub(async () => {
            ...
            await updateGithubInstallationSettings({
                ...
                fail_pr_on_severity: severityByInstallation[installation.id] ?? installation.fail_pr_on_severity,
            });
            await refetchGithubInstallations();
            setGithubSuccessMessage(`GitHub settings updated for installation ${installation.github_installation_id}.`);
        }, { errorMessage: "Failed to save GitHub installation settings." });
    };
    const toggleRepoSelection = (installationId, repositoryFullName, checked) => {
    ...
    return (<div className="mt-10 px-6">
      <Card className="mb-4 lg:m-8">
        ...
        <CardContent className="space-y-6">
          {profileError && (<Alert variant="destructive">
              <AlertDescription>{profileError}</AlertDescription>
            </Alert>)}
          {profileSuccessMessage && (<Alert>
          ...
          <form className="space-y-4" onSubmit={onSaveProfile}>
            ...
            <div className="space-y-2">
              ...
              <Input id="timezone" placeholder="e.g., UTC, Asia/Kolkata" value={timezone} onChange={(e) => setTimezone(e.target.value)}/>
            </div>
            <div className="space-y-2">
            ...
            <div className="space-y-2">
              ...
              <Select value={region} onValueChange={(value) => setRegion(value === "IN" || value === "PK" ? value : "OTHER")}>
      ...
      <Card className="mb-4 lg:m-8">
        ...
        <CardContent className="space-y-6">
          {notificationError && (<Alert variant="destructive">
              <AlertDescription>{notificationError}</AlertDescription>
            </Alert>)}
          {notificationSuccessMessage && (<Alert>
          ...
          <form className="space-y-4" onSubmit={onLoadNotificationSettings}>
            <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
              <div className="w-full space-y-2">
                ...
                <Input id="projectKey" placeholder="e.g., owner/repo" value={projectKey} onChange={(event) => setProjectKey(event.target.value)}/>
              </div>
          ...
          <form className="space-y-4" onSubmit={onSaveNotificationSettings}>
            <div className="space-y-4 rounded-md border border-border/60 p-4">
              <div className="flex items-start gap-3">
                <Checkbox id="emailOnScanComplete" checked={emailOnScanComplete} onCheckedChange={(checked) => setEmailOnScanComplete(checked === true)} disabled={isNotificationLoading}/>
              ...
              <div className="flex items-start gap-3">
                <Checkbox id="emailOnVulnerability" checked={emailOnVulnerability} onCheckedChange={(checked) => setEmailOnVulnerability(checked === true)} disabled={isNotificationLoading}/>
                <div className="space-y-1">
              ...
              <div className="flex items-start gap-3">
                <Checkbox id="weeklyDigest" checked={weeklyDigest} onCheckedChange={(checked) => setWeeklyDigest(checked === true)} disabled={isNotificationLoading}/>
                <div className="space-y-1">
      ...
      <Card className="mb-4 lg:m-8">
        ...
        <CardContent className="space-y-6">
          {scannerAccessError && (<Alert variant="destructive">
              <AlertDescription>{scannerAccessError}</AlertDescription>
            </Alert>)}
          {scannerAccessSuccessMessage && (<Alert>
          ...
          <div className="space-y-4 rounded-md border border-border/60 p-4">
            <div className="flex flex-wrap items-center gap-2">
              ...
              {snykCredentialSource && (<Badge variant="outline">Credential source: {snykCredentialSource}</Badge>)}
            </div>
            ...
            <div className="space-y-1 text-sm">
              ...
              {snykReadyReason && (<div className="text-muted-foreground">Reason: {snykReadyReason}</div>)}
              {scannerHealth && (<div className="text-muted-foreground">
                  Health probes:
                  {" "}
            ...
            <form className="space-y-4" onSubmit={onSaveScannerAccessSettings}>
              <div className="space-y-2">
                ...
                <Input id="snykApiKey" type="password" placeholder={snykApiKeyAttached ? "Enter new key to replace current one, or leave blank to clear" : "Enter Snyk API key"} value={snykApiKey} onChange={(event) => setSnykApiKey(event.target.value)}/>
              </div>
      ...
      <Card className="mb-4 lg:m-8">
        ...
        <CardContent className="space-y-6">
          {githubError && (<Alert variant="destructive">
              <AlertDescription>{githubError}</AlertDescription>
            </Alert>)}
          {githubSuccessMessage && (<Alert>
          ...
          <div className="space-y-2 rounded-md border border-border/60 p-4">
            ...
            {githubAppSetup?.configured ? (<div className="space-y-3">
                <p className="text-sm text-muted-foreground">
                  Install the GitHub App in GitHub first, then link the resulting installation to this workspace.
                </p>
                <div className="flex flex-col gap-3 sm:flex-row sm:items-center">
                  {githubAppSetup.install_url ? (<Button asChild type="button">
                      <a href={githubAppSetup.install_url} target="_blank" rel="noreferrer">
                        Install GitHub App
                        <ExternalLink className="ml-2 h-4 w-4"/>
                      </a>
                    </Button>) : (<div className="rounded-md border border-dashed border-border/60 p-3 text-sm text-muted-foreground">
              ...
              </div>) : (<div className="rounded-md border border-dashed border-border/60 p-3 text-sm text-muted-foreground">
            ...
            <form className="flex flex-col gap-3 sm:flex-row sm:items-end" onSubmit={onLinkGithubInstallation}>
              <div className="w-full space-y-2">
                ...
                <Input id="githubInstallationId" placeholder="e.g. 12345678" value={githubInstallationIdInput} onChange={(event) => setGithubInstallationIdInput(event.target.value)}/>
                <p className="text-xs text-muted-foreground">
          ...
          {isGitHubInstallationsLoading && (<div className="text-sm text-muted-foreground">Loading GitHub installations...</div>)}

          {!isGitHubInstallationsLoading && githubInstallations.length === 0 && (<div className="rounded-md border border-border/60 p-4 text-sm text-muted-foreground">
          ...
          {githubInstallations.map((installation) => {
            ...
            return (<div key={installation.id} className="space-y-4 rounded-md border border-border/60 p-4">
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <Button type="button" onClick={() => void onSaveGithubInstallationSettings(installation)} disabled={isGitHubLoading}>
                ...
                <div className="grid gap-4 lg:grid-cols-2">
                  <div className="space-y-2 rounded-md border border-border/60 p-4">
                    ...
                    <div className="flex items-start gap-3">
                      <Checkbox id={`push-${installation.id}`} checked={pushTriggerByInstallation[installation.id] ?? installation.trigger_on_push} onCheckedChange={(checked) => setPushTriggerByInstallation((current) => ({
                    ...current,
                    ...
                    <div className="flex items-start gap-3">
                      <Checkbox id={`pr-${installation.id}`} checked={prTriggerByInstallation[installation.id] ?? installation.trigger_on_pr} onCheckedChange={(checked) => setPrTriggerByInstallation((current) => ({
                    ...current,
                    [installation.id]: checked === true,
                }))}/>
                      <div className="space-y-1">
                  ...
                  <div className="space-y-4 rounded-md border border-border/60 p-4">
                    <div className="space-y-2">
                      ...
                      <Input id={`branches-${installation.id}`} placeholder="main, develop" value={targetBranchesByInstallation[installation.id] ?? installation.target_branches.join(", ")} onChange={(event) => setTargetBranchesByInstallation((current) => ({
                    ...current,
                    ...
                    <div className="space-y-2">
                      ...
                      <Select value={severityByInstallation[installation.id] ?? installation.fail_pr_on_severity} onValueChange={(value) => setSeverityByInstallation((current) => ({
                    ...current,
                ...
                <div className="space-y-3 rounded-md border border-border/60 p-4">
                  ...
                  <div className="grid gap-3 md:grid-cols-2">
                    {availableRepos.map((repositoryFullName) => (<div key={repositoryFullName} className="flex items-start gap-3 rounded-md border border-border/50 p-3">
                        <Checkbox id={`${installation.id}-${repositoryFullName}`} checked={selectedRepos.includes(repositoryFullName)} onCheckedChange={(checked) => toggleRepoSelection(installation.id, repositoryFullName, checked === true)}/>
                        <div className="space-y-1">
