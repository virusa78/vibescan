...
export class QuotaService {
    async publishQuotaEvent(userId, type, quota) {
        ...
        await publishAndRouteCanonicalEventSafely(createCanonicalEventInput(type, 'quota.service', {
    ...
    async publishPostConsumeSignals(userId, quota) {
    ...
    async getQuota(userId, tx = null) {
        ...
        if (user.quotaResetDate && user.quotaResetDate <= now) {
            ...
            if (!tx) {
                ...
                await this.recordLedgerEntry(userId, 'monthly_reset', 0, // amount doesn't matter for reset
                user.monthlyQuotaUsed, 0, 'automatic_monthly_reset');
     ...
     */
    async consumeQuota(userId, scanId, tx) {
        if (!tx) {
     ...
     * @returns Updated quota info
     */
    async refundQuota(userId, scanId, reason) {
     ...
     * @private
     */
    async recordLedgerEntry(userId, action, amount, balanceBefore, balanceAfter, reason) {
        try {
