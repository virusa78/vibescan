...
function getCompletedScanners(scan) {
    return scan.scanResults.map((result) => result.source);
}
function getPlannedSources(scan) {
    return Array.isArray(scan.plannedSources) && scan.plannedSources.length > 0
        ? scan.plannedSources
        : null;
}
function buildFailureUpdate(scan, scannerId, errorMessage) {
    ...
    const hasOtherExpectedSuccess = otherExpectedScanners.some((source) => completedScanners.includes(source));
...
async function publishScanDomainEvents(prisma, input) {
    ...
    const countsBySource = scanResults.reduce((acc, result) => {
    ...
    await publishAndRouteCanonicalEventSafely(createCanonicalEventInput(input.status === 'done' ? 'scan.completed' : 'scan.failed', 'scan.lifecycle', {
        scanId: input.scanId,
        status: input.status,
        findingsCount: input.findingsCount ?? totalCount,
        errorMessage: input.errorMessage ?? null,
    }, baseExtras), `scan.${input.status}`);
    if (input.status !== 'done') {
        return;
    }
    await publishAndRouteCanonicalEventSafely(createCanonicalEventInput('scanner.comparison.completed', 'scan.lifecycle', {
        scanId: input.scanId,
    ...
    }, baseExtras), 'scanner.comparison.completed');
    await publishAndRouteCanonicalEventSafely(createCanonicalEventInput('scanner.delta.computed', 'scan.lifecycle', {
        scanId: input.scanId,
        deltaCount: scanDelta?.deltaCount ?? 0,
        deltaBySeverity: scanDelta?.deltaBySeverity ?? {},
        isLocked: scanDelta?.isLocked ?? false,
    }, baseExtras), 'scanner.delta.computed');
    if (winnerEntry) {
        await publishAndRouteCanonicalEventSafely(createCanonicalEventInput('scanner.winner.updated', 'scan.lifecycle', {
            scanId: input.scanId,
            winnerScanner: winnerEntry[0],
            winnerCount: winnerEntry[1],
            findingsByScanner: countsBySource,
        }, baseExtras), 'scanner.winner.updated');
    }
}
async function loadScanWithResults(prisma, scanId) {
...
export async function finalizeScanIfReady({ prisma, scanId, userId, findingsCount, loggerLabel, }) {
...
export async function handleScannerFailure({ prisma, scanId, userId, scannerId, errorMessage, loggerLabel, }) {
    ...
    const statusErrorMessage = typeof statusUpdate.errorMessage === 'string' ? statusUpdate.errorMessage : undefined;
...
//# sourceMappingURL=scanLifecycleService.js.map