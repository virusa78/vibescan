...
function getLegacyCompatibilityTotals(totalsBySource) {
...
function extractUnifiedStats(scanResults) {
    for (const result of scanResults) {
        if (!result.rawOutput || typeof result.rawOutput !== 'object')
            continue;
        const ingestionMeta = result.rawOutput.ingestionMeta;
        if (!ingestionMeta || ingestionMeta.resultStatus !== 'ingested')
            continue;
        const unifiedStats = ingestionMeta.unifiedStats;
        if (!unifiedStats || typeof unifiedStats !== 'object')
            continue;
        if (typeof unifiedStats.vulnerabilityCount !== 'number')
            continue;
        return {
            vulnerabilityCount: unifiedStats.vulnerabilityCount,
            severityCounts: (unifiedStats.severityCounts || {}),
        };
    }
    return null;
}
export const getReport = async (rawArgs, context) => {
    const { scanId } = ensureArgsSchemaOrThrowHttpError(getReportInputSchema, rawArgs);
    ...
    const canAccessScan = scan.workspaceId === user.workspaceId ||
    ...
    const annotationByFindingId = new Map(acceptanceRows.map((acceptance) => [
    ...
    const effectiveSeverityBreakdown = unifiedStats
        ? {
