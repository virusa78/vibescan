...
import { getGitHubInstallationMetadata, listGitHubInstallationRepositories, } from './githubAppService';
function toBigIntInstallationId(value) {
...
async function findByInstallationId(installationId) {
...
export async function syncGithubInstallation(installationId, overrides) {
...
export async function removeGithubInstallation(installationId) {
    const normalizedInstallationId = toBigIntInstallationId(installationId);
...
export async function linkGithubInstallationToWorkspace(installationId, workspaceId, orgId) {
...
export async function getMappedGithubInstallationByInstallationId(installationId) {
    const normalizedInstallationId = toBigIntInstallationId(installationId);
    return findByInstallationId(normalizedInstallationId);
}
export async function listGithubInstallationsForWorkspace(workspaceId) {
    const records = await prisma.githubInstallation.findMany({
        where: { workspaceId },
        orderBy: { createdAt: 'desc' },
    });
    const availableReposByInstallation = new Map();
    await Promise.all(records.map(async (record) => {
        try {
            const repositories = await listGitHubInstallationRepositories(record.githubInstallationId);
            availableReposByInstallation.set(record.id, repositories.map((repo) => repo.fullName).sort());
        }
        catch {
...
export async function updateGithubInstallationSettings(installationId, workspaceId, settings) {
