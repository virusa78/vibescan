...
- **2026-05-26**: Normalized project target reference resolution for file upload scans to ignore the `upload-[digits]-` timestamp prefix, resolving multiple scans of the same file to the same project. Added a `onlyLatestScan` filter to the findings overview query (enabled by default) along with a "Hide duplicate findings" toggle on the Findings page UI. Unified `ScannerLineupCard.tsx` design by adding left-hand colored stripes across all card modes, standardizing control layouts to the top-right, and aligning grid columns.